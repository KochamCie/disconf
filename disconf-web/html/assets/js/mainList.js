(function ($) {
    ...
    function getMachineList(machinelist) {
        var tip;
        if (null == machinelist || machinelist.length == 0) {
            tip = "";
        } else {
        ...
        $.ajax({
            ...
            function (data) {
                if (data.success === "true") {
                    ...
                    console.log("fetchConfigValue"+new Date()+",result:"+result.value)
                    var e = object;
                    e.popover('destroy')
                    e.popover({
                        trigger: 'hover',
    ...
    // 回滚信息
    //
    function rollback(configId, object) {

        // 增加确认框
        var confirm = window.confirm("确认回退版本内容吗？");
        if(!confirm){
            return;
        }
        ...
        $.ajax({
            type: "PUT",
            url: "/api/web/config/rollback/" + configId
        }).done(
            function (data) {
                if (data.success === "true") {
                    var result = data.result;
